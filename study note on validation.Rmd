...
car_augment = car %>% 
  ...
  mutate(model_1_pred = predict(model_1,newdata = car %>% filter(split == "holdout"),type = "response")) %>% 
  mutate(model_2_pred = predict(model_2,newdata = car %>% filter(split == "holdout"),type = "response"))
...
car_augment_bins = car_augment %>% 
  mutate(bin = ntile(model_1_pred,10)) %>% #UPDATE TO BE EQUAL BINS BY EXPOSURE, NOT ROW COUNT
  arrange(model_1_pred) #not needed but to check that ntile worked

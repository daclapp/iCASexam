...
WeightedGini = function(actual,weights,predicted){
 ...
 # predicted = predicted
  ...
  temp = temp %>% 
    mutate(random_key = runif(nrow(temp))) %>%  # create random number sort key so ties will be resolved in random order  
...
LorenzCurve_DC <- function (data = NULL, actual = NULL, weights = NULL, predicted = NULL, plot = FALSE)
{
 ...
 data = data %>% 
   filter(!is.na(actual)) %>% 
   arrange_(predicted) %>% 
   mutate(totalPo sitive = actual * weights)
...
LorenzCurve_DC(data = train_augment, actual = "weight", weights = "count", predicted = "weight_p")
...
test_pgin <- round(with(test_augment,WeightedGini(weight, count, weight_p)), 5)
...
train_pgin <- round(with(train_augment,
 WeightedGini(actual = weight, weights = count, predicted = weight_p)), 5)
